...
func (l *HttpLoader) Load(url string) ([]byte, error) {
	...
	if resp.StatusCode != 200 {
		return nil, &HttpLoaderError{"NotOk", errors.New(strconv.Itoa(resp.StatusCode)), url}
	}
	if resp.ContentLength > l.maxsize {
		return nil, &HttpLoaderError{"TooBig", errors.New(fmt.Sprintf("%d", resp.ContentLength)), url}
	}
